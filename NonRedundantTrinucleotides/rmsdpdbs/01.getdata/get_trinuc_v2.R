...
mainDir <- "/orozco/projects/PDB.datamining/rmsd_data2/"
  ...
  if(length(ind_break) > 0){
    for(i in 1:length(ind_break)){
      ...
      if(length(ntinfo[ind_break[i] - 1, "ntID"]) == 0){
        id_dssr[(ind_break[i]):(ind_break[i]+1),"Good"] <- FALSE
        #Check where the ind_break is found at the the end
      }else if(is.na(ntinfo[ind_break[i] + 1,"ntID"])){
        id_dssr <- id_dssr[(ind_break[i]-1):(ind_break[i]),"Good"] <- FALSE
      }else{
        id_dssr[(ind_break[i]-1):(ind_break[i]+1),"Good"] <- FALSE
      }
    }
  }
